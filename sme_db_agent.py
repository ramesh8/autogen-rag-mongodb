...
query_check_agent = autogen.AssistantAgent(
        ...
        "You have to check the given query is valid and passes the QA using the provided tool. if query passed validation just say TERMINATE.",
        llm_config=llmconfig,
        human_input_mode="NEVER",
        is_termination_msg=lambda msg: "terminate" in msg["content"].lower(), 
    )
...
group_chat_result = user_proxy.initiate_chat(
    manager, message=f"Find 10 questions in  Java"
)